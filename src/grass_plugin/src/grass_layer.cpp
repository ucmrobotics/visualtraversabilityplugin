...
#include "nav2_grass_costmap_plugin/grass_layer.hpp"
...
void GrassLayer::onInitialize()
{
  ...
  combination_method_ = 0;           // update with overwrite: every valid value from this layer is written into the master grid (does not copy NO_INFORMATION)
  default_value_ = NO_INFORMATION;   // not to overwrite real obstacles
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &GrassLayer::dynamicParametersCallback,
  ...
  double * min_x, double * min_y, double * max_x, double * max_y)
{
  ...
  // get the clearing observations and update the current status
  current_ = getClearingObservations(clearing_observations);
...
void GrassLayer::raytraceFreespace(
...
void GrassLayer::updateOrigin(double new_origin_x, double new_origin_y)
{
...
PLUGINLIB_EXPORT_CLASS(nav2_costmap_2d::GrassLayer, nav2_costmap_2d::Layer)